(function() {
    ...
    function calificarEmparejamiento(event) {
        ...
        if (puntaje == 100) {
            ...
            $('.btns_modal').html('<button type="button" class="btn" data-dismiss="modal" style="font-size: 20px;color: white; background:#A48E39;;font-weight: bold;margin-top: 1%;" onclick="reiniciar()">cerrar</button>');
        } else {
            $('.img_res').html('<img src="../img/img7/mal.png" style="max-width: 80%;margin-top: 5%;">');
            $('.mensaje').text("¡Puedes hacerlo mejor!");
            $('.puntaje').text(Math.round(puntaje) + "%");
            $('.btns_modal').html('<button style="font-size: 20px;color: white; background:#A48E39;font-weight: bold;margin-top: 1%;" id="add" class="btn calificacion-intentar" data-dismiss="modal" onclick="reiniciar()">Volver a intentar</button>');
        }
